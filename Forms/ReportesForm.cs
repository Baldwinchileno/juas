...
namespace AdminSERMAC.Forms
{
    public partial class ReportesForm : Form
    {
        ...
        private void ConfigureComponents()
        {
            ...
            cmbTipoReporte.Items.AddRange(new object[] { "Por Categoría", "Por Producto","Stock Critico","Productos por vencer" });
        ...
        private async void BtnGenerar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                // Manejar los nuevos tipos de reportes
                switch (tipoReporte)
                {
                    case "Stock Crítico":
                        var stockCritico = await _reportGenerator.GetStockCriticoAsync();
                        dgvResultados.DataSource = stockCritico;
                        return;

                    case "Productos por Vencer":
                        var porVencer = await _reportGenerator.GetProductosPorVencerAsync();
                        dgvResultados.DataSource = porVencer;
                        return;
                ...
                // Mostrar el diagnóstico antes de generar los reportes normales
                var diagnostico = await _reportGenerator.DiagnosticQuery();
                MessageBox.Show(diagnostico, "Diagnóstico de Datos");

                // Obtener el período y determinar el tipo de reporte
                var periodo = cmbPeriodo.SelectedItem.ToString();
                var esPorCategoria = tipoReporte == "Por Categoría";

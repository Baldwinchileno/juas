...
public class VentasForm : Form
{
    ...
    private async void MostrarPreciosHistoricos(string codigo, string rut)
    {
        try
        {
            stockLabel.Text = $"Stock disponible: {productosPorCodigo[codigo].Kilos:N2} kg";
            stockLabel.ForeColor = productosPorCodigo[codigo].Kilos > 0 ? Color.DarkGreen : Color.Red;
            ...
            preciosLabel.Text = $"Último precio de compra: {(ultimoPrecioCompra.HasValue ? ultimoPrecioCompra.Value.ToString("C0") : "No disponible")}";
        }
        catch (Exception ex)
        {
            MessageBox.Show($"Error al obtener precios históricos: {ex.Message}", "Error",
                MessageBoxButtons.OK, MessageBoxIcon.Error);
        }
    }
    ...
    private async void FinalizarButton_Click(object sender, EventArgs e)
    {
        ...
        try
        {
            ...
            foreach (DataGridViewRow row in ventasDataGridView.Rows)
            {
                ...
                var precio = Convert.ToDouble(row.Cells["Precio"].Value); // Añadimos esta línea
    ...
    INSERT INTO Ventas (
        ...
        @kilosNeto, @fechaVenta, @pagadoConCredito, @rut, @total, @precio
                ...
                command.Parameters.AddWithValue("@precio", precio); // Añadimos esta línea
